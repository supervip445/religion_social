...
/**
 * onlineUsers = Map<user_id_string, socket_id>
 */
...
app.post("/api/notify", (req, res) => {
  ...
  if (to_user_id === "public" || to_user_id === "all") {
    ...
    return res.json({
      ...
      users_notified: onlineUsers.size,
    });
  }

  // Send to specific user (normalize to string key)
  const targetSocketId = onlineUsers.get(String(to_user_id));
  if (targetSocketId) {
...
io.on("connection", (socket) => {
  ...
  socket.on("register", (user_id) => {
    ...
    console.log(`✅ User registered: user_id=${key}, socket=${socket.id}`);
  ...
  socket.on("send_noti", (data) => {
    ...
    if (to_user_id === "public" || to_user_id === "all") {
      console.log(
        `📢 Broadcasting notification to all ${onlineUsers.size} users: ${
  ...
  socket.on("disconnect", () => {
    ...
    for (let [uid, sid] of onlineUsers.entries()) {
      if (sid === socket.id) {
        ...
        console.log(
          `👋 User ${uid} disconnected. Remaining users: ${onlineUsers.size}`
        );
        break;
      }
    }
